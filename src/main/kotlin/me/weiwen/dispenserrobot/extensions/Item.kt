...
fun ItemStack.applyDamage(damage: Int): Boolean {
    val itemMeta = itemMeta ?: return true
    if (itemMeta.isUnbreakable) {
        return true
    }

    val unbreaking = itemMeta.getEnchantLevel(Enchantment.UNBREAKING)
...
fun ItemStack.applyDamage(damage: Int, container: Container): Boolean {
    val slot = container.inventory.first(this)
    if (slot == -1) {
        return false
    }

    if (this.applyDamage(damage)) {
